...
const animate = () => {
  ...
  loader
    ...
    .add('#logo #T', a.sideLetterAnimation('left'), prefersReducedMotion.value ? 0 : 2000)
    .add('#logo #L', a.sideLetterAnimation('right'), prefersReducedMotion.value ? 0 : 2000)
...
<template>
  <section id="loader" class="fixed inset-0 z-50 size-full overflow-y-hidden bg-black">
    <div id="logo-wrapper" class="flex min-h-svh flex-col place-content-center">
      <LazySvgoLogo id="logo" class="h-24 w-full brightness-200" hydrate-on-visible />
      <div v-if="showLoadingProgress" class="absolute left-1/2 mt-72 -translate-x-1/2">
